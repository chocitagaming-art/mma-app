...
// Entidades con nombre que aparecen de verdad en los datos scrapeados. `&amp;`
// va la ÚLTIMA a propósito: decodificarla antes convertiría `&amp;#039;` en
// `&#039;` y de ahí en `'` dentro de la MISMA pasada, saltándose el tope de dos.
...
/**
 * Decodifica las entidades HTML que los scrapers dejan crudas en la BD.
 *
 * NO es teórico: medido el 28-jul-2026 contra la BD de producción, **54 de las
 * 1.127** filas con `trains_at` traen entidades (`Bronx&#039;s Gold Team`,
 * `Glory MMA &amp; Fitness`) y 2 de 1.515 `birth_place`, una de ellas
 * DOBLEMENTE codificada (`Ponte dell&amp;#039;Olio`). Los nombres de luchador,
 * apodos, nacionalidades y todos los campos de evento están limpios (0 filas),
 * pero se pasan igual por aquí: es una única puerta de saneado para todo lo que
 * viaja a Google, y así un scraper que empiece a ensuciar no nos pilla.
 *
 * Dos pasadas por el doble codificado, y no más: es el máximo observado y un
 * bucle sin tope sería un juguete para quien controle el dato de entrada.
 */
